...
| **지금 검색이 어떤 상태인가 · 뭘 채택했나** | **[[검색 실험 결과]]** |
| **실험이 뭘 기준으로 재는 건가** (파이프라인·정답지·지표·통제) | **[[검색 실험 설계]]** ← 측정 조건 정본 |
| 어떤 모델·변형을 놓고 비교하나 (M0/M1/M2, C~G) | [[검색 실험 설계]] **§6~§8** |
| 정답지가 공정한가 · 라벨을 어떻게 늘렸나 | [[검색 실험 결과]] **§8** |
| 실험하다 뭘 틀렸고 어떻게 알았나 | [[검색 실험 결과]] **2부** |
...
**실험은 3단계로 갔다** — **후보 구성 → 리랭커 → 임베딩**. 리랭커·임베딩은 후보 안에 든 것만 다루므로 아래층이 바뀌면 위층 판정이 다시 열리는데, **1단계에서 후보가 C(dense 단독)로 확정**되어 위층 조건이 그대로다. **세 판정 모두 최종** → [[검색 실험 설계]] §8-1

| 문서 | 답하는 것 | 상태 |
|---|---|---|
| **[[검색 실험 설계]]** | 파이프라인·전제 · 정답지(v5) · 지표 · 실험 대상(M0~M2, R1·R2, C~G) · 통제 조건 | **★ 조건 정본** |
| **[[검색 실험 결과]]** | 판정표 · 프로덕션 반영 상태 · **실험하며 바로잡은 것 4건** | **★ 판정 정본** |
...
| **정답 청크 (v5)** | **연인원 353** | v4의 310 + 신규 42 + 검증 추가 1 → [[검색 실험 결과]] §8 |
...
> 라벨을 늘린 경위 → [[검색 실험 결과]] §8

### 검색 성적 — 값 정본은 [[검색 실험 결과]] §1
...
| 문서 | 대체 정본 | 왜 내렸나 |
|---|---|---|
| `검색 개선.md` | [[검색 실험 설계]] · [[검색 실험 결과]] | **1부 구현 커밋 이력은 여기가 유일한 기록** — 그 목적으로만 |
| `검색 실험 계획 (164문항).md` | [[검색 실험 설계]] | 36~164문항 시절 설계 |
| `검색 실험 결과 (164문항).md` | [[검색 실험 결과]] | 유효한 내용은 전부 이관 |
| `검색 2차 실험 (임베딩·BM25·리랭커).md` | 〃 | "1차·2차" 프레이밍을 버리며 재편 |
| `검색 2차 실험 계획/결과 *.md` | 〃 | 그 전 판 (**변형 D 설명이 틀림**) |
| `정답지 편향 제거 (전체 풀링 라벨링).md` | [[검색 실험 결과]] §8·§9 | 상·하한 3가정 표 전체가 여기 남아 있다 |
| `검색 실험 구도.md` | [[검색 실험 설계]] | 측정 전 사전등록 설계서 |
| `검색 품질 개선 (BM25·리랭커).canvas` | [[검색 실험 설계]] | 4변형 시절 설계도 |
...
- **BM25 재측정 진행 중** (3단계 재판정의 **1단계** — 후보 구성 확정이 먼저다 → [[검색 실험 설계]] §8-1) — 리랭커 위에서 잰 하이브리드 수치가 **설계 결함 2건으로 전부 무효**다(변형이 후보 집합을 안 바꿈 · **dense는 문서 안인데 BM25는 전체**를 봄). 스코프를 맞춰 다시 재는 중 → [[검색 실험 결과]] §4·§10
- **`retrieve.py`의 "문서가 특정되면 BM25를 끈다"가 옳았나** — 지금 재는 것이 그 답이다 → [[검색 실험 결과]] §4-2
- **정답이 후보에 아예 없는 문항** — dense만으로는 e5 17 / KURE 9. 임베딩으로는 다 못 없앤다. 위 BM25 재측정이 이 영역을 겨냥한다
- **라벨링 보류 5건** — 사람 확인 대상. 쟁점은 표기 정규화(예: "3개월"과 "90일"을 같은 값으로 볼지) → [[검색 실험 결과]] §8-4
- **KURE는 언제 다시 보나** — 지금은 기각이지만 조건 3개 중 하나면 재판정한다(골든셋 300문항↑ · 다른 이유로 재인덱싱 · Recall@20이 병목인 기능) → [[검색 실험 결과]] §2-1
...
- ~~리랭커를 아직 안 켰다~~ → **`RETRIEVAL_USE_RERANKER = True`로 켰다.** 선결 둘 다 해소 → [[검색 실험 결과]] §5
...
- ~~리랭커 모델끼리는 비교한 적이 없다~~ → **R1 유지 확정.** 한국어 튜닝판 R2는 전 조합 −0.06~−0.08. 점수가 1.0으로 포화해 변별을 못 한다 → [[검색 실험 결과]] §3
- ~~M0×R2에서 C_doc ≠ D_doc (0.001 차이)~~ → **버그 아님.** 애초에 **D가 후보 집합을 안 바꿔 C와 동치**이고, 0.001은 R2 점수 동점 위에 얹힌 부수 현상이다 → [[검색 실험 결과]] §10-1
